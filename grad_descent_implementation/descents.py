...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function is LossFunction.MSE:
            ...
            mask = (np.abs(diff) <= 1)
            less_delta = (diff[mask]**2 / 2).sum()
            more_delta = (np.abs(diff[~mask]) - 0.5).sum()
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        if self.loss_function is LossFunction.MSE:
            return -2 / y.shape[0] * (x.T @ y - x.T @ (x @ self.w))
        elif self.loss_function is LossFunction.LogCosh:
            ...
            diff = self.predict(x) - y
            mask = (np.abs(diff) <= 1)
            less_delta = x[mask].T @ diff[mask]
            more_delta = x[~mask].T @ np.sign(diff[~mask])
            return 1 / y.shape[0] * (less_delta + more_delta)
...
class Adamax(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        m_hat = self.m / (1 - self.beta_1**self.iteration)
        weight_diff = -self.lr() / self.u * m_hat
        self.w += weight_diff
        return weight_diff
